    ...
    <div class="inside card" ref="inside">
      ...
      <div class="body">
        <div class="card white" ref="body">
          <div class="info">
            <div>
              <div class="title" v-if="type === 'moment'">
                标题: {{ title }}
              </div>
              <div class="type">类型: {{ type }}</div>
              <div class="ctime">
                创建时间: {{ parseDate(this.Time.created) }}
                {{ parseTime(this.Time.created) }}
              </div>
              <div class="comment" v-if="type === 'picture' && content.comment">
                注释: {{ content.comment }}
              </div>
            </div>
            <div class="icon">
    ...
    active(val) {
      if (val === false) {
        ...
        this.$refs.wrap.setAttribute(
          'style',
          'height: ' + (98 + this.overflowHeight) + 'px'
        )
        this.$refs.inside.setAttribute(
          'style',
          'transform: translateY(0px);height: calc(' +
